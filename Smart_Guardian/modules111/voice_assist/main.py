...
class ChatApp(QWidget):
    ...
    def initUI(self):
        ...
        self.startButton = QPushButton('开始聊天')
        self.startButton.clicked.connect(self.startChat)
    ...
    def startChat(self):
        ...
        if os.path.exists("./output_auto.wav"):
            try:
                text = gettext()
                self.textArea.append("检测到录音如下：\n" + text)
                if("天气" in text):
                    answer=output_weather()
                else:
                    answer = TYgraphPrompt(text)
                self.textArea.append("AI回答如下：\n" + answer)
                output(answer)
            except Exception as e:
                self.play_error_audio()
                QMessageBox.warning(self, '错误', '发生了错误: {}'.format(e))
        else:
            self.textArea.append("没有找到录音文件！")
